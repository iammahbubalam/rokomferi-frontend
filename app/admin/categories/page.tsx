...
import { FolderTree, Plus, Save, CheckCircle, LayoutList, TrendingUp, Eye, EyeOff } from 'lucide-react';
...
export default function AdminCategoriesPage() {
  ...
  const filteredItems = items.filter(item => {
    switch (activeView) {
      ...
      case 'nav': return item.showInNav;
      case 'hidden': return !item.showInNav;
      default: return true;
    }
  });
  ...
  const tabs = [
    { id: 'all' as ViewTab, label: 'Total Categories', count: totalCount, color: 'from-blue-500 to-blue-600', icon: LayoutList },
    { id: 'active' as ViewTab, label: 'Active', count: activeCount, color: 'from-green-500 to-green-600', icon: TrendingUp },
    { id: 'inactive' as ViewTab, label: 'Inactive', count: totalCount - activeCount, color: 'from-red-500 to-red-600', icon: EyeOff },
    { id: 'nav' as ViewTab, label: 'In Navigation', count: navCount, color: 'from-purple-500 to-purple-600', icon: Eye },
    { id: 'hidden' as ViewTab, label: 'Hidden from Nav', count: hiddenCount, color: 'from-amber-500 to-amber-600', icon: EyeOff },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Click cards below to filter view</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            {isDirty && (
              <Button onClick={handleSave} className="bg-green-600 hover:bg-green-700 text-white shadow-lg px-6">
            ...
            return (
              <button
                ...
                className={`bg-gradient-to-br ${tab.color} rounded-xl p-5 text-white shadow-lg transition-all duration-200 text-left ${
                  isActive ? 'ring-4 ring-offset-2 ring-gray-400 scale-[1.02]' : 'opacity-80 hover:opacity-100 hover:scale-[1.01]'
                }`}
              >
